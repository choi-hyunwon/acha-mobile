...
/* scss TASK*/
function scss() {
	return gulp.src("src/scss/style/*.scss").pipe(sourcemaps.init()).pipe(sass().on("error", sass.logError)).pipe(sourcemaps.write()).pipe(gulp.dest("dist/css/style"));
}
...
function jsLib() {
	...
	return gulp.src(sourceLib).pipe(concat("front.lib.js")).pipe(gulp.dest("dist/js/lib"));
}

function jsCommon() {
	let sourceUi = ["src/js/ui/*.js"];
	return gulp.src(sourceUi).pipe(concat("front.common.js")).pipe(gulp.dest("dist/js/ui"));
}
...
function beautify() {
	...
	return gulp.src("./dist/html/*.html").pipe(htmlbeautify(options)).pipe(gulp.dest("./dist/html/"));
}
